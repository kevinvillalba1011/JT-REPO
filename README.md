...
- **Anti-colisión de PDFs destino:** los PDFs finales (individual en `OCR_DESTINATION_PATH`, masivo en `EXCEL_DESTINATION_PATH`) se guardan dentro de una subcarpeta `yyyyMMddHHmmss` (fecha+hora Bogotá), y si dos oficios generan el mismo `nombreOficioFinal` en esa misma subcarpeta (mismo segundo), el segundo recibe sufijo `_1`, `_2`, etc. — el `nombreOficioFinal` persistido en DB no lleva ese sufijo, solo el archivo físico.
