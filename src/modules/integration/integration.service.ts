...
export class IntegrationService {
  ...
  async getToken(): Promise<string> {
    ...
    const authPayloadStr = this.configService.get<string>('INTEGRATION_AUTH_PAYLOAD', '{}');
    ...
    try {
      ...
      this.logger.error('Failed to parse INTEGRATION_AUTH_PAYLOAD environment variable.');
      payload = {};
    }
    ...
    if (!response.ok) {
      ...
      this.logger.error(`Authentication failed: ${response.status} - ${errorText}`);
      throw new Error(`Failed to authenticate with external REST service: ${response.statusText}`);
    }
  ...
  async sendData(finalJson: any, source: string): Promise<boolean> {
    ...
    if (!dataUrl) {
      this.logger.warn('INTEGRATION_DATA_URL not configured. Skipping external integration dispatch.');
      return false;
    }
    ...
    try {
      ...
      this.logger.log(`Sending JSON data to external service for source [${source}].`);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        this.logger.error(`Failed to send data to REST service: ${response.status} - ${errorText}`);
        return false;
      }

      this.logger.log(`Successfully sent data from [${source}] to external API.`);
      return true;
    } catch (error: any) {
      this.logger.error(`Integration error during [${source}]: ${error.message}`);
      return false;
    }
  }
}
