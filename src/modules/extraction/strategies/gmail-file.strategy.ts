...
export class GmailFileStrategy implements FileExtractorStrategy {
  ...
  async extractFiles(destinationFolder: string): Promise<ExtractedFile[]> {
    ...
    const host = this.configService.get<string>('GMAIL_IMAP_HOST', 'imap.gmail.com');
    const port = this.configService.get<number>('GMAIL_IMAP_PORT', 993);
    const searchSubject = this.configService.get<string>('GMAIL_SEARCH_SUBJECT', 'DOCUMENTO_PROCESAR');

    if (!user || !pass) {
      this.logger.warn('Gmail credentials not configured. Skipping extraction.');
      return [];
    }
      ...
      try {
        ...
        for (const seq of messages) {
          ...
          if (allowedExtensions.length > 0 && !allowedExtensions.includes(ext)) {
            this.logger.debug(`Skipping Gmail attachment ${filename}: Extension ${ext} not allowed.`);
            continue;
          }
